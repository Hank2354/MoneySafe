...
class NewExpenseInteractor: NewExpenseInteractorType {

    var presenter: NewExpensePresenterType?
    ...
    func fetchUserSelectedCategories() -> Result<[GeneralCategoryModel], Errors> {
        ...
        case .success(let userSettings):
            ...
            let userExpenseCategories = userSettings.expenseCategoriesID
            
            guard let userExpenseCategories = userExpenseCategories else { return .failure(Errors.loadUserSettingsError) }
            
            for userExpenseCategory in userExpenseCategories {
                
                for expenseCaregory in expenseCaregories {
                    
                    if expenseCaregory.categoryID == userExpenseCategory {
